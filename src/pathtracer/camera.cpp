...
Ray Camera::generate_ray(double x, double y) const {
  ...
  /*
    Function takes normalized (x, y) coordinates in image space.

    Outputs a "Ray" in the world space.

    First transform the image coordinates to camera space.

    Generate the ray in camera space.

    Transform it into a ray in the world space. A ray has origin and a direction.  You need to transform both to convert
    to world space correctly.

    Code stuff:
      hFov, vFov - members that define the virtual sensor. In degrees, not radians.

      pos - camera position in the world space.

      c2w - 3x3 camera-to-world rotation matrix.

      nclip, fclip - double precision floating point positive values representing near and far clipping planes. Use them
      to initialize min_t and max_t of Ray respectively.
   */

  //Compute the coordinates in camera space:
  double cx, cy;

  cx = x - 0.5;
  cy = y - 0.5;

  cx = 2* cx * tan(radians(hFov / 2));
  cy = 2 * cy * tan(radians(vFov / 2));

  Vector3D world_d = c2w*Vector3D(cx, cy, -1);
  world_d.normalize();

  Ray ray = Ray(pos, world_d);
  ray.min_t = nClip;
  ray.max_t = fClip;
